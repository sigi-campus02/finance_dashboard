...
def safe_remove_index(apps, schema_editor):
    """Drop the legacy index only if it still exists - Database-agnostic"""
...
def safe_rename_index(apps, schema_editor):
    """Rename the legacy index only if it is still present - Database-agnostic"""
    connection = schema_editor.connection

    # SQLite unterstützt kein ALTER INDEX RENAME
    if connection.vendor == 'sqlite':
        return  # Skip für SQLite
